...
public void generateLogicalAnd(BlockScope currentScope, CodeStream codeStream, boolean valueRequired) {
	...
	// default case
	boolean custom = (TypeBinding.notEquals(this.right.resolvedType, TypeBinding.BOOLEAN) && this.right.resolvedType.id !=TypeIds.T_JavaLangBoolean) || (TypeBinding.notEquals(this.left.resolvedType, TypeBinding.BOOLEAN)  && this.left.resolvedType.id !=TypeIds.T_JavaLangBoolean);
	if (custom) {
		generateOperatorOverloadCode(currentScope, codeStream, valueRequired);
	}else{
		this.left.generateCode(currentScope, codeStream, valueRequired);
...
public TypeBinding resolveType(BlockScope scope) {
	//if (this.resolvedType!=null) return this.resolvedType;	TODO Test1 ??
	...
	operatorSignature = OperatorExpression.OperatorSignatures[operator][(leftTypeID << 4) + rightTypeID];
	if(this.appropriateMethodForOverload == null){
